...
class DataBaseSamplerV2:
        ...
        if len(sampled) > 0:
            ...
            if 1 > gt_drop_rate > 0:
                ...
                for i in range(len(s_points_list)):
                    ...
                    mask = mask >= gt_drop_rate
                    s_points_list[i] = s_points_list[i][mask]
                    counts[i] = len(s_points_list[i])
                mask2keep = counts >= gt_drop_max_keep
            elif gt_drop_rate >= 1:
                counts = np.zeros((num_sampled,))
                for i in range(len(s_points_list)):
                    mask = np.zeros((len(s_points_list[i]),), dtype=np.bool_)
                    sweeps = np.unique(s_points_list[i][:, -1])
                    if gt_drop_rate >= 10:
                        nsweeps = min(len(sweeps), np.random.randint(1, 11))
                    else:
                        nsweeps = min(len(sweeps), int(gt_drop_rate))
                    sweeps = np.random.choice(sweeps, nsweeps, replace=False)
                    for sw in sweeps:
                        mask |= s_points_list[i][:, -1] == sw
                    s_points_list[i] = s_points_list[i][mask]
                    counts[i] = len(s_points_list[i])
                mask2keep = counts >= gt_drop_max_keep
            else:
    ...
    def sample_class_v3(self, name, num, gt_boxes):
        target_num = num
        valid_samples = []
        while len(valid_samples) < target_num:
            sampled = self._sampler_dict[name].sample(num)
            sampled = copy.deepcopy(sampled)
            num_gt = gt_boxes.shape[0]
            num_sampled = len(sampled)
            ...
            for i in range(num_gt, num_gt + num_sampled):
                if coll_mat[i].any():
                    ...
                    valid_samples.append(sampled[i - num_gt])
            num = target_num - len(valid_samples)
        return valid_samples
