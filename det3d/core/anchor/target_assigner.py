...
class TargetAssigner:
    ...
    def assign(
        ...
        if anchors_mask is not None:
            def prune_anchor_fn(_): return np.where(anchors_mask)[0]
        else:
    ...
    def assign_v2(
        ...
        for class_name, anchor_dict in anchors_dict.items():
            ...
            if anchors_mask is not None:
                ...
                def prune_anchor_fn(_): return np.where(anchors_mask_class)[0]
            else:
                prune_anchor_fn = None

            targets = create_target_np(
                anchor_dict["anchors"].reshape(-1, self.box_coder.n_dim),
                gt_boxes[mask],
                similarity_fn,
                box_encoding_fn,
                prune_anchor_fn=prune_anchor_fn,
