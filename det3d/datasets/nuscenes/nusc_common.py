...
def combine_sweeps(info, nsweeps):
    ...
    # for n sweeps
    prev_lidar_path = str(info['lidar_path'])
    for i in range(nsweeps - 1):
        sweep = info["sweeps"][i]
        # if repeating sweeps comment below
        # if str(sweep["lidar_path"]) == prev_lidar_path:
        #     prev_lidar_path = str(sweep["lidar_path"])
        #     continue
        prev_lidar_path = str(sweep["lidar_path"])
    ...
    info['lidar_path'] = info['lidar_path'].replace('samples/', 'samples_{:02d}'.format(nsweeps))
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=10, filter_zero=True):
    ...
    for sample in tqdm(nusc.sample):
        ...
        ref_cam_back_path, _, _ = nusc.get_sample_data(ref_cam_back_token)

        ref_cam_backleft_token = sample["data"]["CAM_BACK_LEFT"]
        ref_cam_backleft_path, _, _ = nusc.get_sample_data(ref_cam_backleft_token)

        ref_cam_frontleft_token = sample["data"]["CAM_FRONT_LEFT"]
        ref_cam_frontleft_path, _, _ = nusc.get_sample_data(ref_cam_frontleft_token)

        ref_cam_frontright_token = sample["data"]["CAM_FRONT_RIGHT"]
        ref_cam_frontright_path, _, _ = nusc.get_sample_data(ref_cam_frontright_token)

        ref_cam_backright_token = sample["data"]["CAM_BACK_RIGHT"]
        ref_cam_backright_path, _, _ = nusc.get_sample_data(ref_cam_backright_token)
        ...
        info = {
            ...
            "cam_frontleft_path": ref_cam_frontleft_path,
            "cam_frontright_path": ref_cam_frontright_path,
            "cam_back_path": ref_cam_back_path,
            "cam_backleft_path": ref_cam_backleft_path,
            "cam_backright_path": ref_cam_backright_path,
